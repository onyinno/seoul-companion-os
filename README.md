...
- 購物項目相片檔案會上傳到 Supabase Storage 私有 bucket：`trip-photos`。
- 目前仍沿用既有 LocalStorage 狀態儲存購物項目 metadata（例如 `storagePath`、`fileName`、`updatedAt`）。
- 因此本版本尚未提供完整跨裝置購物項目同步；完整 database sync 將於後續 PR 實作。
