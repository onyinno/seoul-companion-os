...
export function AddActivitySheet({ open, dayId, mode, initialActivity, onClose, onSubmit }: AddActivitySheetProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-slate-900/40">
      ...
      <section className="absolute bottom-0 left-0 right-0 max-h-[88vh] overflow-hidden rounded-t-3xl bg-white shadow-soft">
        ...
        <form className="space-y-3 overflow-y-auto px-4 py-4" onSubmit={handleSubmit}>
          <label className="block space-y-1 text-sm">
            <span>活動類型</span>
            <select className="w-full rounded-xl border border-slate-300 px-3 py-2" value={category} onChange={(e) => setCategory(e.target.value as ActivityCategory)} required>
          ...
          <label className="block space-y-1 text-sm">
            <span>時間</span>
            <input type="time" className="w-full rounded-xl border border-slate-300 px-3 py-2" value={time} onChange={(e) => setTime(e.target.value)} required />
          </label>

          <label className="block space-y-1 text-sm">
            <span>地點名稱</span>
            <input type="text" className="w-full rounded-xl border border-slate-300 px-3 py-2" value={place} onChange={(e) => setPlace(e.target.value)} required />
          </label>

          <label className="block space-y-1 text-sm">
            <span>備註</span>
            <textarea className="min-h-24 w-full rounded-xl border border-slate-300 px-3 py-2" value={note} onChange={(e) => setNote(e.target.value)} required />
          </label>

          <label className="block space-y-1 text-sm">
            <span>預計花費</span>
            <input type="number" min={0} className="w-full rounded-xl border border-slate-300 px-3 py-2" value={cost} onChange={(e) => setCost(e.target.value)} />
          </label>

          <div className="sticky bottom-0 grid grid-cols-2 gap-2 bg-white pt-2 pb-1">
            <Button type="button" onClick={onClose} className="rounded-xl border border-slate-300 bg-white px-4 py-3 text-slate-700">取消</Button>
            <Button type="submit" className="rounded-xl bg-slate-900 px-4 py-3 text-white">{mode === 'edit' ? '儲存變更' : '儲存'}</Button>
          </div>
        </form>
      </section>
    </div>
